...
Project. 
\item Mettre ``API 19 : Android 4.4'' pour les 3 choix (min SDK, target SDK,
compile with)
\item \textbf{Décocher ``Create custom launcher icon'' et choisir ``Empty
...
\item Lancer l'appliation (au besoin, utiliser click droit sur le projet : run
as :
Android application)
\end{itemize}
\begin{enumerate}
...
\begin{itemize}
\item Créer un nouvel appareil avec la configuration suivante : Nexus one,
Android 4.4.2 - API level 19, Skin with dynamic hardware controls, RAM : 768.
